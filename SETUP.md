...
| `שלה בפק"ע` | סדר הפעולה במסלול — קובע את סדר השלבים |
...
סדר השלבים נלמד מהמסלולים עצמם: המערכת קוראת מכל פק"ע מי בא לפני מי, ומסדרת את
השלבים החדשים לפי הסדר שכל המסלולים מסכימים עליו (עם `שלה בפק"ע` כשובר שוויון).
...
- **צור מספרים סידוריים לפי הכמות** (ברירת מחדל: כבוי) — ב-Priority אין סידוריים,
  ובלעדיהם אי אפשר לדווח עבודה. יש תקרה לכל פק"ע כדי לא לנפח את הנתונים
